...
const getUserCart = async (user_id) => {
	try {
		const cart = await CartModel.findOne({ user_id }).populate('products.details');
		if(cart) return cart;
		return new CartModel({ user_id }).save()
	} catch (error) {
		return Promise.reject({message: `Failed to get user cart: ${error.message}`, status: 400})
	}
}
...
const addToCart = async (product_id, user_id) => {
	try {
		...
		const productCartIndex = products.findIndex(product => product.id.toString() === product_id);
		console.log(productCartIndex)
		// means product is present in cart, so need to update quantity
		if(productCartIndex > -1) {
			return Promise.reject({message: 'Product already added in cart', status: 400})
		} else {
			products.push({
				id: product_id,
				cart_quantity: 1
...
const removeFromCart = async (product_id, user_id) => {
	try {
		const cart = await getUserCart(user_id);
		let products = [...cart.products] || [];
		const productIndexInCart = products.findIndex(product => product.id.toString() === product_id);
		if(productIndexInCart === -1) return Promise.reject({message: 'Product not in cart', status: 400});
